...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form class="form_repeater" action="{{ route('admin.order.store') }}" name="addfrm" id="addfrm"
                        method="POST">
                        @csrf
                        ...
                        <div class="row">
                            {{-- <div class="col-6">
                                <div class="mb-3">
                                    <label class="form-label" for="pay-worker-payment">Pay Worker Payment<span
                                            class="text text-danger h5"> *</span></label>
                                    <input type="text" class="form-control calculate_amount inventory-items-expense-amt input-mask text-start" data-inputmask="'alias': 'numeric', 'groupSeparator': ',', 'digits': 2, 'digitsOptional': false, 'prefix': '$ ', 'placeholder': '0'"
                                        name="pay_worker_payment" id="pay_worker_payment">
                                    @error('pay_worker_payment')
                                        <span class="text text-danger" role="alert">
                                            {{ $message }}
                                        </span>
                                    @enderror
                            ...
                            {{-- <div class="col-4">
                                <div class="mb-3">
                                    <label class="form-label" for="select-device">Select Device<span class="text text-danger h5"> *</span></label>
                                    <select class="form-select select2" name="inventory_id[]" id="inventory_id" multiple>
                                    </select>
                                    @error('inventory_id')
                                        <span class="text text-danger" role="alert">
                                            {{ $message }}
                            ...
                            {{-- <div class="col-4">
                                <div class="mb-3">
                                    <label class="form-label" for="selling amount">Selling amount<span
                                            class="text text-danger h5"> *</span></label>
                                    <input type="tel" class="form-control input-mask text-start"
                                        data-inputmask="'alias': 'numeric', 'groupSeparator': ',', 'digits': 2, 'digitsOptional': false, 'prefix': '$ ', 'placeholder': '0'"
                                        name="sell_amount" id="sell_amount" placeholder="Enter sell amount">
                                    @error('sell_amount')
                                        <span class="text text-danger" role="alert">
                                            {{ $message }}
                                        </span>
                                        ...
                                        <div data-repeater-item class="row mb-3" style="display:flex;align-items: end;">
                                            <div class="col-5 select_device">
                                                <label class="form-label" for="select-device">Select Device<span
                                                        class="text text-danger h5"> *</span></label>
                                                <select required class="form-select inventory_id" name="inventory_id">
                                                </select>
                                                @error('inventory_id')
                                                    <span class="text text-danger" role="alert">
                                                        {{ $message }}
                                            ...
                                            <div class="col-5">
                                                <label class="form-label" for="inventory-amount">Amount<span
                                                        class="text text-danger h5"> *</span></label>
                                                <input required type="text" class="form-control input-mask    item_amount calculate_amount text-start" name="per_amount"
                                                data-inputmask="'alias': 'numeric', 'groupSeparator': ',', 'digits': 2, 'digitsOptional': false, 'prefix': '$ ', 'placeholder': '0'"/>
                                                @error('per_amount')
                                                    <span class="text text-danger" role="alert">
                                                        {{ $message }}
                                            ...
                                            <div class="col-2">
                                                ...
                                                <button data-repeater-delete type="button" class="view_invetory btn btn-outline-danger">
                                        ...
                                        <div data-repeater-item class="row mb-3" style="display:flex;align-items: end;">
                                            ...
                                            <div class="col-5">
                                                <label class="form-label" for="damage-amount">Expense Amount<span
                                                        class="text text-danger h5"> *</span></label>
                                                <input required type="text" class="form-control input-mask damage_amount calculate_amount inventory-items-expense-amt text-start" name="damage_amount"
                                                data-inputmask="'alias': 'numeric', 'groupSeparator': ',', 'digits': 2, 'digitsOptional': false, 'prefix': '$ ', 'placeholder': '0'"/>
                                                @error('damage_amount')
                                                    <span class="text text-danger" role="alert">
                                                        {{ $message }}
                                            ...
                                            <div class="col-2">
                                                <button data-repeater-delete type="button" class="view_invetory btn btn-danger">
        ...
        function inventory_html_render($data, renderDom) {
            ...
            if ($data.expenses.length > 0) {
                $data.expenses.map(($expenses) => {
                    html_expense += `<p style="margin-bottom: 0px;"><i class="bx bx-chevrons-right font-size-16 align-middle text-primary me-1"></i>${ $expenses.expense_name } - $${parseFloat($expenses.pivot.amount).toFixed(2)}</p>`;
            ...
            if ($data.phone_damages.length > 0) {
                $data.phone_damages.map(($phone_damage) => {
                    html_phone_notes += `<p class="text-muted" style="margin-bottom: 0px;"><i class="bx bx-chevrons-right font-size-16 align-middle text-primary me-1"></i> ${ $phone_damage.damage_name }</p>`;
                });
            }
